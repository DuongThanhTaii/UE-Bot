...
export class DefaultModelsService implements ModelsService {
  ...
  private async getRequiredEngine(
    ...
    throw new Error(
      `${provider} engine is not ready. Please restart UE Bot so bundled extensions can reload.`
    )
  }
  ...
  async pullModel(
    ...
    const engine = await this.getRequiredEngine()
