...
export class DefaultThreadsService implements ThreadsService {
  async fetchThreads(): Promise<Thread[]> {
    if (isSyncApiEnabled()) {
      ...
      return syncApiClient.listThreads(token, guestSessionId)
    }
  ...
  async createThread(thread: Thread): Promise<Thread> {
    if (isSyncApiEnabled()) {
      const { token, guestSessionId } = useAuth.getState()
      return syncApiClient.createThread(thread, token, guestSessionId)
    }
  ...
  async updateThread(thread: Thread): Promise<void> {
    if (isSyncApiEnabled()) {
      const { token, guestSessionId } = useAuth.getState()
      await syncApiClient.updateThread(thread, token, guestSessionId)
      return
    }
  ...
  async deleteThread(threadId: string): Promise<void> {
    if (isSyncApiEnabled()) {
      const { token, guestSessionId } = useAuth.getState()
      await syncApiClient.deleteThread(threadId, token, guestSessionId)
      return
    }
