...
export function DataProvider() {
  ...
  const completeOAuthFromUrl = useAuth((state) => state.completeOAuthFromUrl)
  ...
  useEffect(() => {
    ...
    const syncDefaultProductivityServer = async () => {
      const shouldActivate =
        voice.jiraCalendarServerEnabled && productivity.enabled
      ...
      const nextConfig = {
        ...
        env: {
          ...
          JIRA_OAUTH_CLIENT_ID: productivity.jiraOAuthClientId,
          JIRA_OAUTH_CLIENT_SECRET: productivity.jiraOAuthClientSecret,
          GOOGLE_ACCESS_TOKEN: productivity.googleAccessToken,
          GOOGLE_OAUTH_CLIENT_ID: productivity.googleOAuthClientId,
          GOOGLE_OAUTH_CLIENT_SECRET: productivity.googleOAuthClientSecret,
          GOOGLE_CALENDAR_ID: productivity.googleCalendarId,
          GOOGLE_CALENDAR_BASE_URL: productivity.googleCalendarBaseUrl,
          PRODUCTIVITY_OAUTH_PORT: '38973',
        },
        active: shouldActivate,
      }

      const currentConfig =
        useMCPServers.getState().mcpServers['productivity-mcp']
      if (JSON.stringify(currentConfig) === JSON.stringify(nextConfig)) {
        return
      }

      useMCPServers.getState().addServer('productivity-mcp', nextConfig)
      await useMCPServers.getState().syncServers()
      ...
      if (
        ...
        toast.warning('Productivity MCP is configured with missing secrets', {
          description:
            'Mở cài đặt Voice & ESP32 và hoàn tất thông tin đăng nhập Jira/Google trước khi sử dụng các công cụ trò chuyện.',
        })
      }
    }

    void syncDefaultProductivityServer().catch((error) => {
      console.error('Không thể đồng bộ hóa máy chủ MCP:', error)
    })
    ...
    productivity.googleOAuthClientId,
    productivity.googleOAuthClientSecret,
    productivity.jiraApiToken,
    productivity.jiraBaseUrl,
    productivity.jiraEmail,
    productivity.jiraOAuthClientId,
    productivity.jiraOAuthClientSecret,
    voice.jiraCalendarServerEnabled,
  ])

  useEffect(() => {
    console.log('Đang khởi tạo DataProvider...')
    serviceHub
      .providers()
      .getProviders()
      .then((providers) => {
    ...
    serviceHub
      ...
      .catch((error) => {
        console.warn(
          'Không thể tải các trợ lý ảo, vẫn giữ nguyên mặc định:',
          error
        )
      })
  ...
  }, [initializeAuth, completeOAuthFromUrl])
  ...
  }, [serviceHub, setThreads, authToken, isSyncEnabled])
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      console.log('Đang thực hiện kiểm tra cập nhật định kỳ')
      checkForUpdate()
    ...
    if (enableOnStartup) {
      ...
      serviceHub
        ...
        .then(async (isRunning) => {
          if (isRunning) {
            console.log('Máy chủ API cục bộ đã đang chạy')
            ...
            await Promise.allSettled(
              modelsToStart.map(async ({ model, provider: providerName }) => {
                ...
                try {
                  ...
                  console.warn(
                    `Failed to auto-start server model ${model}:`,
                    err
                  )
                }
              })
        ...
        .catch((error: unknown) => {
          console.error(
            'Không thể khởi động Máy chủ API cục bộ khi khởi động hệ thống:',
            error
          )
