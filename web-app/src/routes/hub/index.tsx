...
function HubContent() {
  ...
  const isJanhqModel = useCallback((model: CatalogModel) => {
    const developer = model.developer?.trim().toLowerCase()
    return developer === 'janhq' || model.model_name.toLowerCase().includes('jan-')
  }, [])

  const getDeveloperLabel = useCallback((developer?: string) => {
    return developer?.trim().toLowerCase() === 'janhq' ? 'UE Bot' : developer
  }, [])
  ...
  const sortedModels = useMemo(() => {
    ...
    return sorted.sort(
      (a, b) => Number(isJanhqModel(a)) - Number(isJanhqModel(b))
    )
  }, [isJanhqModel, sortSelected, sources])
