...
function General() {
  ...
  const isVietnamese = t('common:language') === 'Ngôn ngữ'
  ...
  const handleInstallCli = async () => {
    ...
    try {
      ...
      toast.success(
        isVietnamese
          ? `Đã cài UE Bot CLI vào ${s.path}`
          : `UE Bot CLI installed to ${s.path}`
      )
    } catch (e) {
      toast.error(
        isVietnamese ? 'Cài đặt thất bại' : 'Install failed',
  ...
  const handleUninstallCli = async () => {
    ...
    try {
      ...
      setCliPath(null)
      toast.success(
        isVietnamese ? 'Đã gỡ UE Bot CLI' : 'UE Bot CLI uninstalled'
      )
    } catch (e) {
      toast.error(
        isVietnamese ? 'Gỡ cài đặt thất bại' : 'Uninstall failed',
        { description: String(e) }
      )
    } finally {
      setIsCliLoading(false)
      ...
      <div className="flex h-[calc(100%-60px)]">
        ...
        <div className="p-4 pt-0 w-full overflow-y-auto">
          <div className="flex flex-col justify-between gap-4 gap-y-3 w-full">
            ...
            <Card title={t('settings:general.advancedTitle')}>
              {IS_TAURI && (
                <CardItem
                  title={t('settings:general.cliTitle')}
                  description={
                    cliInstalled && cliPath
                      ? t('settings:general.cliInstalledDesc', {
                          path: cliPath,
                        })
                      : t('settings:general.cliDesc')
                  }
                  actions={
                    cliInstalled ? (
                      <Button
                      ...
                      >
                        {isCliLoading
                          ? t('settings:general.cliUninstalling')
                      ...
                      >
                        {isCliLoading
                          ? t('settings:general.cliInstalling')
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        if (!token) {
                          toast.error(t('settings:general.hfTokenMissing'))
                          return
                        }
                        ...
                        try {
                          ...
                          if (resp.ok) {
                            ...
                            toast.success(t('settings:general.hfTokenValid'), {
                              description: data?.name
                                ? t('settings:general.hfTokenValidAs', {
                                    name: data.name,
                                  })
                                : t('settings:general.hfTokenValidDesc'),
                            })
                          } else {
                            toast.error(t('settings:general.hfTokenInvalid'), {
                              description: t(
                                'settings:general.hfTokenInvalidDesc'
                              ),
                            })
                          }
                        } catch (e) {
                          ...
                          if (name === 'AbortError') {
                            toast.error(t('settings:general.hfTokenTimeout'), {
                              description: t(
                                'settings:general.hfTokenTimeoutDesc'
                              ),
                            })
                          } else {
                            toast.error(t('settings:general.hfTokenFailed'), {
                              description: t(
                                'settings:general.hfTokenFailedDesc'
                              ),
                            })
                          }
                        } finally {
                    ...
                    >
                      {t('settings:general.hfTokenVerify')}
                    </Button>
                  </div>
                }
              />
              ...
              <CardItem
                ...
                description={
                  <div className="text-muted-foreground -mt-2">
                    <p>
                      UE Bot được nghiên cứu và thiết kế bởi 5 sinh viên Đại
                      học Sư phạm, định hướng thành một trợ lý AI local-first
                      cho học tập, làm việc và tự động hóa cá nhân.
                    </p>
                    <p className="mt-2">
                      Cảm ơn llama.cpp, Tauri, Hugging Face và cộng đồng mã
                      nguồn mở đã tạo nên hệ sinh thái để UE Bot có thể chạy
                      mạnh trên máy tính cá nhân.
                    </p>
                  </div>
                }
              />
