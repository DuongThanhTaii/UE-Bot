...
const builtinExtensionLoaders: Record<
  string,
...
export class ExtensionManager {
  ...
  async load() {
    ...
    results.forEach((result) => {
      if (result.status === 'rejected') {
        console.error('Extension onLoad failed:', result.reason)
      }
    })
  }
  ...
  async activateExtension(extension: Extension) {
    ...
    try {
      const builtinLoader = builtinExtensionLoaders[extension.name]
      const extensionClass = builtinLoader
        ? await builtinLoader()
        : await import(
            /* @vite-ignore */ getServiceHub().core().convertFileSrc(
              extension.url
            )
          )
      ...
      if (
        ...
        console.error(`Extension '${extension.name}' has no default export`)
      }
    } catch (error) {
