...
const FRIENDLY_MODEL_NAMES: Record<string, string> = {
  [RECOMMENDED_MODEL_HF_REPO]: RECOMMENDED_MODEL_DISPLAY_NAME,
  [JAN_CODE_HF_REPO]: 'UE Bot Code 4B',
  [JAN_V2_VL_MODEL_HF_REPO]: 'UE Bot Vision',
}

const formatDownloadName = (name: string) => {
  if (/^janhq[\\/]+jan-v3/i.test(name)) {
    return name.replace(/^janhq[\\/]+jan-v3[^/\\]*/i, 'UE Bot/UE-Bot-v1')
  }

  if (FRIENDLY_MODEL_NAMES[name]) {
    return FRIENDLY_MODEL_NAMES[name]
  }

  for (const [repoId, displayName] of Object.entries(FRIENDLY_MODEL_NAMES)) {
    if (name.startsWith(`${repoId}/`) || name.startsWith(`${repoId}\\`)) {
      return name.replace(repoId, displayName)
    }
  }

  return name.replace(/^janhq[\\/]+/i, 'UE Bot/')
}
...
export function DownloadManagement() {
  ...
  const downloadProcesses = useMemo(() => {
    ...
    const downloadsWithProgress = Object.values(downloads).map((download) => ({
      id: download.name,
      name: formatDownloadName(download.name),
    ...
    const localDownloadsWithoutProgress = Array.from(localDownloadingModels)
      ...
      .map((modelId) => ({
        id: modelId,
        name: formatDownloadName(modelId),
        progress: 0,
        current: 0,
        total: 0,
      }))
