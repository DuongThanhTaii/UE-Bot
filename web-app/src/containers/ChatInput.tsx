...
const ChatInput = memo(function ChatInput({
  ...
  const pcSpeechSupported = useVoiceAssistant((state) => state.pcSpeechSupported)
  const pcSpeechEnabled = useVoiceAssistant((state) => state.pcSpeechEnabled)
  const pcListening = useVoiceAssistant((state) => state.pcListening)
  const togglePcListening = usePcSpeech((state) => state.toggleListening)
        ...
        <div className="absolute z-20 bg-transparent bottom-0 w-full p-2 ">
          <div className="flex justify-between items-center w-full">
            ...
            <div className="flex items-center gap-2">
              {pcSpeechSupported && pcSpeechEnabled && !isStreaming && (
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      variant={pcListening ? 'secondary' : 'outline'}
                      size="icon-sm"
                      className="rounded-full mr-1 mb-1"
                      onClick={togglePcListening}
                    >
                      <IconMicrophone
                        size={16}
                        className={cn(
                  ...
                  <TooltipContent>
                    <p>{pcListening ? 'Stop microphone' : 'Speak to UE Bot'}</p>
                  </TooltipContent>
                </Tooltip>
              )}
