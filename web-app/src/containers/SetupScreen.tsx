  ...
  const handleQuickStart = useCallback(() => {
    ...
    void serviceHub
      ...
      .catch((error) => {
        ...
        console.error('SetupScreen quick start failed:', error)
  ...
  useEffect(() => {
    if (
      ...
      void serviceHub
        ...
        .catch((error) => {
          console.error('Queued setup download failed:', error)
          toast.error('Failed to continue model download', {
            description:
          ...
          <div className="shrink-0 px-10 w-[480px] mx-auto overflow-auto pb-10 pointer-events-auto -mt-20">
            <div className="mb-4">
              <h1 className="font-studio font-medium text-2xl mb-1">
                {isDownloading
                  ? 'Hãy chờ một chút, UE Bot đang sẵn sàng...'
                  : 'Hello, Chào mừng bạn đã đến với UE Bot!'}
              </h1>
              <p className="text-muted-foreground leading-normal w-full mt-1">
                {isDownloading
                  ? 'Quá trình này có thể mất vài phút.'
                  : 'UE Bot cần một mô hình cục bộ để bắt đầu. Hãy thiết lập nó.'}
              </p>
            </div>
            <div className="flex gap-4 flex-col mt-6 relative z-50">
              <div className="w-full text-left">
                    ...
                    <div className="flex flex-col w-full h-full justify-center">
                      <div className="flex flex-1 items-center justify-between">
                        <h1 className="font-semibold text-sm mb-1">
                          <span>{RECOMMENDED_MODEL_DISPLAY_NAME}</span>&nbsp;
                ...
                <div className="flex flex-col relative z-50 items-start gap-2 mt-4">
                  ...
                  >
                    {isDownloading ? 'Đang tải về' : 'Tải về'}
                  </Button>
                </div>
              </div>
            </div>
